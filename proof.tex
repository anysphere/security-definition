...
\textbf{Consistent Prefix}: First, we show that $\Pi_{\asphr}$ satisfies \cref{defn:messaging-integrity}, and the consistent prefix property in \cref{defn:messaging-correctness}.
\begin{lemma}
...
is either equal to $\bot$, or equal to some other message user $j$ encrypted using $\sk_r$. Since user $j$ only encrypts messages and ACKS to user $i$ using $\sk_r$, we conclude that either $\ack = \ACK(R_{t'})$ for some $t' \leq t - 1$, or $\ack$ is equal to a previous labeled message user $j$ sent to user $j$. 

If $\ack$ is equal to a labeled message or $\bot$, client $i$ will skip step (7c) and (7d), and no variable will be changed. Now consider the case
$$\ack = \ACK(R_{t'}).$$
In step (7.c), we have $C_i.\frienddb[\reg_j].\seqs = S_{t - 1}$. By the induction hypothesis, we have
$$S_{t - 1} \geq R_{t - 1} \geq R_{t'}.$$
So no variable is changed unless $S_{t - 1} = R_{t - 1} = R_{t'}$, in which case $S_t = R_{t - 1} + 1$, and after popping $\msg^{lb}_{ij}(S_{t - 1})$ we get
...
With this choice of $f$, we describe the full $\cA_1^O$. The step numbers below refer to the steps in \cref{defn:messaging-real-world-experiment} unless otherwise indicated. For any $1 \leq i \leq j \leq N$, denote $\sk_{ij, r} = \sk_{ji, w} = \sk_{i * N + j}$ and $\sk_{ij, w} = \sk_{ji, r} = \sk_{j * N + i}$ as in line (1) of \cref{defn:AE-eval-security}.
\begin{itemize}
    \item Simulate step (1), (2), (3), and (4.b) identical to \cref{defn:messaging-real-world-experiment}.
    \item After step (2), for each pair of $i, j \in \cH$, $\cA_1$ "set"\footnote{What we mean by set is that $\cA_1$ simulates the honest users as if $\gensharedsecret(\reg_i, \reg_j) = \sk_{ij}$. $\cA_1$ does not have access to $\sk_{ij}$.} 
