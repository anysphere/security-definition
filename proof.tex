...
We show that $\Pi_{\asphr}$ satisfies both \cref{defn:messaging-correctness} and \cref{defn:messaging-integrity}.
...
Let $N'(\lambda) = N(\lambda)^2$ and $R'(\lambda) = 2N'(\lambda)T(\lambda)$. Let $\Sim_{\sym}$ be a simulator satisfying \cref{defn:AE-eval-security} and with parameters $(N, R) = (N'(\lambda), R'(\lambda))$, and let $\Sim_{\pir}$ be a simulator satisfying \cref{defn:PIR-SIM-security} with parameters $(N, R) = (N(\lambda), R'(\lambda))$. 
